...
uint32_t
Interest::getNumberChunks() const
{
  return m_number_chunks;
}

//set and get of NumberChunks
Interest&
Interest::setNumberChunks(const uint32_t numberChunks)
{
  m_number_chunks = numberChunks;
  m_wire.reset();
  return *this;
}
...
operator<<(std::ostream& os, const Interest& interest)
{
  ...
  if (interest.getNumberChunks()) {
  os << delim << "ndn.chunks=" << interest.getNumberChunks();
  delim = '&';
  }
