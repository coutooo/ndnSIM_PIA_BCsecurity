...
#########################
data = read.table("/home/couto/Desktop/ndnSIM/ns-3/src/ndnSIM/examples/graphs/rate-trace.txt", header=T)
...
g.root <- ggplot(data.root) +
  ...
  geom_line(aes (x=Time, y=Kilobits.sum, color=Type), linewidth=0.5) +
  ylab("Rate [Kbits/s]")

print(g.root)

png("/home/couto/Desktop/ndnSIM/ns-3/src/ndnSIM/docs/source/_static/root-rates.png", width=500, height=250)
print(g.root)
retval <- dev.off()
