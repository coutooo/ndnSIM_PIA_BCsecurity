...
Forwarder::onIncomingInterest(const Interest& interest, const FaceEndpoint& ingress)
{
  ...
  if (hasDuplicateNonceInPit) {
    // goto Interest loop pipeline
    this->onInterestLoop(interest, ingress);
    m_strategyChoice.findEffectiveStrategy(*pitEntry).afterReceiveLoopedInterest(ingress, interest,
                                                                                 *pitEntry);
    return;
  }
                              ...
                              const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  m_strategyChoice.findEffectiveStrategy(*pitEntry).afterReceiveInterest(interest,
                                                                         FaceEndpoint(ingress.face,
                                                                                      0),
                                                                         pitEntry);
}
                             ...
                             const shared_ptr<pit::Entry>& pitEntry, const Data& data)
{
  if (interest.isPush()) {
    ...
    m_strategyChoice.findEffectiveStrategy(*pitEntry).beforeSatisfyInterest(data,
                                                                            FaceEndpoint(*m_csFace,
                                                                                         0),
                                                                            pitEntry);
...
Forwarder::onIncomingData(const Data& data, const FaceEndpoint& ingress)
{
  ...
  for (const auto& pitEntry : pitMatches) {
    ...
    m_strategyChoice.findEffectiveStrategy(*pitEntry).satisfyInterest(pitEntry, ingress, data,
                                                                      satisfiedDownstreams,
                                                                      unsatisfiedDownstreams);
  ...
  for (const auto& downstream : satisfiedDownstreams) {
    if (downstream.first->getId() == ingress.face.getId() && downstream.second == ingress.endpoint
                          ...
                          const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  if (egress.getLinkType() != ndn::nfd::LINK_TYPE_POINT_TO_POINT) {
    ...
    //               << egress.getId() << " nack=" << pitEntry->getInterest().getName() << "~"
  ...
  const auto affectedEntries =
    this->getNameTree().partialEnumerate(prefix,
                                         [&](const name_tree::Entry& nte) -> std::pair<bool, bool> {
                                           // we ignore an NTE and skip visiting its descendants if
                                           // that NTE has an associated FIB entry (1st condition),
                                           // since in that case the new nexthop won't affect any
                                           // PIT entries anywhere in that subtree, *unless* this is
                                           // the initial NTE from which the enumeration started
                                           // (2nd condition), which must always be considered
                                           if (nte.getFibEntry() != nullptr
                                               && nte.getName().size() > prefix.size()) {
                                             return {false, false};
                                           }
